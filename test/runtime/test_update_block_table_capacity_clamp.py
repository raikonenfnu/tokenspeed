...
This file also covers the scheduler/Python page-table mirror contract:
``occupied_pages`` is the authoritative logical row, while
``new_occupied_pages`` is only an append delta.
...
def _make_forward_op(
    ...
    if occupied_pages is None:
        occupied_pages = new_occupied_pages
...
def test_update_block_table_does_not_raise_on_overflow(monkeypatch):
    ...
    # Rows refresh from position 0; the offender is clamped to table width.
...
def test_update_block_table_passthrough_when_no_overflow(monkeypatch):
    """When no request overflows, copied row sizes survive untouched."""
    from tokenspeed.runtime.execution import cache_loc_kernel
...
def test_update_block_table_uses_authoritative_occupied_pages(monkeypatch):
    """Append deltas are insufficient when an earlier logical page changed."""
    from tokenspeed.runtime.execution import cache_loc_kernel

    req_to_page = torch.zeros(8, 513, dtype=torch.int32)
    # The append delta says only "copy page 12 at begin=2", but the scheduler's
    # full row also replaced logical pages 0 and 1. Python must mirror the full
    # occupied_pages row or it can keep stale/aliased prefix pages.
    forward_op = _make_forward_op(
        begins=[2],
        sizes=[1],
