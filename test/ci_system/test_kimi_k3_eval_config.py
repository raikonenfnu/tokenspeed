...
def test_kimi_k3_eval_is_seeded_and_decode_graph_only():
    ...
    assert "--force-deterministic-rsag" in server_args
    assert "--no-enable-prefix-caching" in server_args
    ...
    generation_config = json.loads(
        _option_values(eval_args, "--generation-config")[0]
    )
...
def test_kimi_k3_evalscope_version_is_pinned():
    config = yaml.safe_load(CONFIG_PATH.read_text())
    assert any(
        "evalscope[perf]==1.9.1" in command
        for command in config["eval"]["install"]
    )
